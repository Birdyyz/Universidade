...
```{r}
"dizer que um estimador θ^ é não-enviasado para um parametro θ significa que quando a dimensão da amostra aumenta a estimativa tenta aproximar-se do verdadeiro valor do parâmetro. Quanto maior a amostra tende a aproximar-se do verdadeiro valor do parametro "
```
...
```{r}
"porque é um estimador centrado, consistente, a média amostral também e é um estimador eficiente (a variancia é pequena)"
```
...
```{r}
"O verdadeiro valor esta em 95% dos valores da amostra???"
```
...
```{r}
"quando o tamanho amostral aumenta o comprimento do intervalo fica mais estreito(a amplitude do intervalo diminui)"
```
---
...
"
A proporcao esta entre 0.38 e 0.52
"
```
...
```{r}
p0 <- 0.4
prop.test(sucessos,n, p = p0, alternative = "two.sided")
# aq tem less greater ou two sided
```
   (e) Indique a sua decisão relativamente a $H_0$ e interprete o resultado no contexto do problema (usando linguagem acessível).
```{r}
" 
Vamos comparar o p-value com o alpha == 0.05
(Rejeitamos H0 sse o p-value for menor q o alpha)
Não rejeitamos a hipotese H0 pq p-value é maior q o alpha


resposta:
nao rejeito h0 para um alpha == 0.05.Logo nao existem evidencias para afirmar que a proporcao de estudantes que usam transportes publicos é diferente de 0.4
"
```
---
