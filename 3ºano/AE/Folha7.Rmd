...
dates <- scale(data)
covariancias <- cov(data)

correlacao <- prcomp(dates,center=TRUE, scale = TRUE)
sum0 <- summary(correlacao)
valprop <- ((sum0$dev)^2)
pe <- ((sum0importance))
cumulative <- pe[3,]
df_pe <- data.frame(
...
screeplot(correlacao, npcs = min(64,6830))
ggplot(df_pes, aes(x = PC, y = cumprop)) +
  geom_point()

biplot(correlacao)
ggplot(correlacao$x , aes(x = PC1, y = PC2, color = NI60$labs))+
  geom_point() +
  labs(
    x = "PC1 (score)",
    y = "PC2 (score)",
...
load_pc1 <- correlacao$rotation[,1]
genes_pc1 <- names(sort(abs(load_pc1), decreasing = TRUE))[1:25]
load_pc2 <- correlacao$roation[,2]
genes_pc2 <- names(sort(abs(load_pc2), decreasing = TRUE))[1:25]
genes_pc1
genes_pc2
